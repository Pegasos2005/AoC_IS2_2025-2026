package software.ulpgc.aoc.reto12.model;
...
public class TimberShape {
    ...
    // Ahora las variaciones son finales y se calculan al inicio (Thread-Safe)
    ...
    // Constructor privado para las variaciones internas (evita recursión infinita de init)
    private TimberShape(int id, boolean[][] mask, boolean isVariation) {
        this.id = id;
        this.mask = mask; // Ya viene recortada por la rotación/flip
    ...
    private boolean[][] trim(boolean[][] input) {
        ...
        for (int r = 0; r < newH; r++) {
            for (int c = 0; c < newW; c++) {
                // Protección contra arrays "jagged" (irregulares)
                int srcRow = minRow + r;
                int srcCol = minCol + c;
